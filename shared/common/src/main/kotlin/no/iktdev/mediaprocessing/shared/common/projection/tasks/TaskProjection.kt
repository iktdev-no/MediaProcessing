...
class TaskProjection(val events: List<Event>) {
    ...
    val readStreamsTaskStatus: TaskStatus = projectStreamReadStatus()
    ...
    val contentStoredTaskStatus: TaskStatus = projectStoreContentAndMetadataStatus()
    val determineCollectionTaskStatus: TaskStatus = projectDeterminedCollectionStatus()

    fun getStatuses() = listOf(
        TaskType.ReadMediaStreams to readStreamsTaskStatus,
        ...
        TaskType.PersistContentInfo to contentStoredTaskStatus,
        TaskType.DetermineContentCollection to determineCollectionTaskStatus,
    )
    fun hasFailed() = getStatuses().any { it.second == TaskStatus.Failed }

    fun operationStatuses(): List<TaskStatus> {
        val required = events.getInstancesOf<StartProcessingEvent>().last().data.operation ?: emptySet()
