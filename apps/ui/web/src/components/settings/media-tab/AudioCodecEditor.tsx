...
export default function AudioCodecEditor({
  codec,
  onChange,
}: {
  ...
  const update = (patch: Partial<AudioCodecConfig>) => onChange({ ...patch });
        ...
        <>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                update({
                  bitrate: e.target.value ? Number(e.target.value) : null,
                })
              }
            />
          </FieldSection>
          ...
          >
            <ChannelSlider
              ...
              max={maxChannels}
              onChange={(v) => update({ channels: Math.min(v, maxChannels) })}
            />
